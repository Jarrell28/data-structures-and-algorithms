...
const count = (target, input) => {
  // Solution code here...
  let sum = input.reduce((acc, numArr) => {
    let newArr = numArr.filter(num => {
...
const totalSum = (input) => {
  // Solution code here...
  let sum = input.reduce((acc, numArr) => {
    let subSum = numArr.reduce((acc, num) => { return acc + num }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const filteredInput = input.map(arr => {
    ...
    return filtered.map(num => Math.pow(2, num));
  })
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.reduce((char, obj) => {
    if (Object.keys(char).length === 0) {
      char = obj;
      return char;
    }

    if (parseInt(char.height) > parseInt(obj.height)) {
      char = obj;
      return char;
    }
