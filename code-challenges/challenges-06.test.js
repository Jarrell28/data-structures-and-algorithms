...
const sortByChildren = (charArray) => {
  ...
  charArray.sort((a, b) => {
    if (a.children.length > b.children.length) {
      ...
      return -1;
    } else if (a.children.length === b.children.length || a.children.length === 0 || b.children.length === 0) {
      if (a.house > b.house) {
        return 1;
      } else {
        return -1;
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let family = Object.values(arr[i]);

    if (family[0] === character || family[1] === character) {
      if (family[2].length > 0) {
        return true;
      } else {
        return false;
      }
